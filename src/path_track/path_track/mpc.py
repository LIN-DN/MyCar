...
class Tracker(Node):
    ...
    def __init__(self, name):
        ...
        self.path_sub = self.create_subscription(Path, '/record_path', self.path_cb, qos_profile)#, callback_group=self.path_cbG
        print("程序初始化就绪!")
    ...
    def vehicle_pose_cb(self, msg):
        ...
        if len(self.path) > 0:
            ...
            self.my_car.dt = round(current_timestamp - self.previous_timestamp, 4)
            print(f"时间差{self.my_car.dt}")
            self.MPC_control()
    ...
    def path_cb(self, msg):
        ...
        self.previous_timestamp = msg.header.stamp.sec + msg.header.stamp.nanosec * 1e-9
    ...
    def MPC_control(self):
        ...
        # print(f"MPC: 加速度{acc} 待转角{np.rad2deg(delta)} 速度{self.my_car.v} ")
        ...
        # print(f"MPC: 加速度{acc} 待转角{np.rad2deg(delta)} 控制线速度{round(vel, 3)} 角速度{np.rad2deg(steer)}")
