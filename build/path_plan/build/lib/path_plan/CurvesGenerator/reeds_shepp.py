...
class PATH:
    def __init__(self, lengths, ctypes, L, x, y, yaw, directions):
        self.lengths = lengths  # lengths of each part of path (+: forward, -: backward) [float list]
        self.ctypes = ctypes  # type of each part of the path [string list]
        ...
        self.directions = directions  # forward: 1, backward:-1 [int list]


# 计算最优路径 找到一条最优的路径，使得该路径的总长度最小
...
# 计算所有可能的路径 生成起点到终点之间的所有可能路径，并将这些路径转换为全局坐标系下的路径
...
# 根据给定的局部路径长度和模式，生成一系列插值点，用于描述机器人在整个路径上的运动轨迹。
# 路径点包括 x 和 y 坐标、偏航角和方向信息，可以在后续的路径规划和控制中使用。
...
# 根据给定的路径段信息，在前一个路径点的基础上计算出当前路径点的坐标、偏航角和方向，并更新路径信息。
...
# 根据起始状态和目标状态之间的差异，利用不同的路径生成函数生成一系列可能的路径
...
'''
    utils 
'''
# 将任意角度值映射到一个等价的角度值，使其处于 [-π, π] 范围内, 确保角度的连续性和一致性。
def pi_2_pi(theta):
    while theta > PI:
        theta -= 2.0 * PI
...
# 将直角坐标系下的点 (x, y) 转换为极坐标系下的 (r, theta) 表示。极坐标可以更方便地描述点的位置和方向，特别适用于需要考虑距离和角度的问题。
def R(x, y):
    """
...
# 根据路径中的运动模式和长度生成一个可读性较高的标签，用于描述路径的运动方式和方向。
def get_label(path):
    label = ""
...
# 根据路径的坐标和方向信息计算路径上各点的曲率，并返回曲率和路径段长度的列表
...
# 在路径规划过程中验证计算得到的路径是否满足一些基本的约束条件，包括起点、终点和路径长度的检查
